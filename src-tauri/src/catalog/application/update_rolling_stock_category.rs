use crate::catalog::domain::railway_model::{
    RailwayModelId, RailwayModelUowExt, RollingStockCategory, RollingStockId,
};
use crate::core::domain::domain_error::DomainError;

/// Input for [`UpdateRollingStockCategory::execute`].
pub struct UpdateRollingStockCategoryInput {
...
/// Use case that changes the category (variant) of a single rolling stock unit.
pub struct UpdateRollingStockCategory;

impl UpdateRollingStockCategory {
    ...
    pub async fn execute<U>(
        unit_of_work: &mut U,
        input: UpdateRollingStockCategoryInput,
    ) -> Result<(), DomainError>
    where
        U: RailwayModelUowExt + Send,
    {
        ...
        model.update_rolling_stock_category(&input.rolling_stock_id, input.category)?;
...
mod tests {
    ...
    #[tokio::test]
    async fn updates_category_successfully() {
        ...
        UpdateRollingStockCategory::execute(
            &mut uow,
            UpdateRollingStockCategoryInput {
    ...
    async fn returns_not_found_when_model_missing() {
        ...
        let err = UpdateRollingStockCategory::execute(
            &mut uow,
            UpdateRollingStockCategoryInput {
                railway_model_id: model_id,
                rolling_stock_id: rs_id,
