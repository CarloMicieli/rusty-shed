<script lang="ts">
  ...
  interface Props {
    ...
    /** Dark mode styling (charcoal/amber design system) */
    dark?: boolean;
  }
...
<div class={dark ? 'overflow-hidden rounded-lg border border-[#1F1F1F] bg-[#0F0F0F] p-4' : ''}>
  <section>
    {#if dark}
    ...
    <div class="space-y-4">
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          <label for="base-manufacturer" class="block space-y-1">
            <span
              class={dark
                ? 'text-[10px] text-[#808080] uppercase'
                : 'text-sm text-muted-foreground'}>{m.add_model_manufacturer()}</span
            >
          </label>
          <select
            id="base-manufacturer"
            bind:value={form.manufacturerId}
            class={dark ? darkInput : lightInput}
        ...
        <div>
          <label for="base-product-code" class="block space-y-1">
            <span
              class={dark
                ? 'text-[10px] text-[#808080] uppercase'
                : 'text-sm text-muted-foreground'}>{m.add_model_product_code()}</span
            >
          </label>
          <input
            ...
            class={dark
              ? `${darkInput} font-mono`
              : 'flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 font-mono text-sm ring-offset-background placeholder:text-muted-foreground focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50'}
      ...
      <div>
        <label for="base-description" class="block space-y-1">
          <span
            class={dark ? 'text-[10px] text-[#808080] uppercase' : 'text-sm text-muted-foreground'}
            >{m.add_model_description()}</span
          >
        </label>
        <input
          id="base-description"
        ...
        <div>
          <label for="base-category" class="block space-y-1">
            <span
              class={dark
                ? 'text-[10px] text-[#808080] uppercase'
                : 'text-sm text-muted-foreground'}>{m.add_model_category()}</span
            >
          </label>
          <select
            id="base-category"
        ...
        <div>
          <label for="base-scale" class="block space-y-1">
            <span
              class={dark
                ? 'text-[10px] text-[#808080] uppercase'
                : 'text-sm text-muted-foreground'}>{m.add_model_scale()}</span
            >
          </label>
          <select
            id="base-scale"
        ...
        <div>
          <label for="base-power-method" class="block space-y-1">
            <span
              class={dark
                ? 'text-[10px] text-[#808080] uppercase'
                : 'text-sm text-muted-foreground'}>{m.add_model_power_method()}</span
            >
          </label>
          <select
        ...
        <div>
          <label for="base-epoch" class="block space-y-1">
            <span
              class={dark
                ? 'text-[10px] text-[#808080] uppercase'
                : 'text-sm text-muted-foreground'}>{m.add_model_epoch()}</span
            >
          </label>
          <select
            id="base-epoch"
  ...
  <section>
    <div class="mb-4 flex items-center justify-between">
      ...
      <Button
        ...
        size="sm"
        class={dark
          ? 'border border-[#1F1F1F] bg-transparent text-[#E0E0E0] hover:bg-[rgba(212,138,66,0.15)]'
          : ''}
