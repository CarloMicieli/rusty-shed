...
<Sheet {open} {onOpenChange} class="border-white/10 bg-background/90 backdrop-blur-xl">
  <div class="p-6 text-white">
    <div class="mb-6">
      <h3 class="font-mono text-sm tracking-widest text-amber-500 uppercase">
      ...
      <div class="rounded-lg border border-white/5 bg-white/5 p-4">
        ...
        <div class="flex items-center justify-between">
          <div class="flex flex-col gap-1">
            ...
            <span class="font-mono text-2xl font-bold text-amber-500">{dccAddress ?? '—'}</span>
          </div>
          <Button class="bg-amber-500 text-black">{m.depot_detail_update_addr()}</Button>
        </div>
      </div>
