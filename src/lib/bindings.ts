...
async cloudBackupDisconnectGoogle(userEmail: string) : Promise<Result<null, CommandError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("cloud_backup_disconnect_google", { userEmail }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type RecordCounts = { manufacturers: number; railwayCompanies: number; railwayModels: number; collectionItems: number; sellers: number; maintenanceCards: number }
/**
 ...
 */
export type UpdateSettingsInput = { currency: string | null; language: Language | null; measureUnit: MeasureUnit | null; favouriteScale: string | null; powerSystem: PowerSystem | null; theme: AppTheme | null }
/**
