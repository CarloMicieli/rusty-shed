<script lang="ts">
	...
	let statusBadge = $derived({
		...
		badgeClass: model.status === 'InCollection'
...
<div class="railway-model-card card gauge-frame ring-1 ring-border/40 {className}">
	<!-- Header Section (T012) -->
	<header class="flex flex-col gap-2 border-b border-white/10 pb-4 md:flex-row md:items-center md:justify-between">
		<div class="flex flex-col gap-1">
			<h2 class="text-lg font-semibold md:text-xl">
				{model.manufacturer} {model.product_code}
			</h2>
	...
	<div
		class="hero-section relative mt-4 aspect-video w-full overflow-hidden rounded-lg border bg-black/20 md:max-w-2xl {isDragging ? 'border-primary bg-primary/10' : 'border-white/10'}"
		role="img"
