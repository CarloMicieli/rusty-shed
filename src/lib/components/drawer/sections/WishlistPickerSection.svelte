<script lang="ts">
  ...
  import { DrawerSectionCard } from '$lib/components/drawer';
...
<DrawerSectionCard label={m.wishlist_modal_choose_or_create()}>
  <div class="space-y-3">
    <Popover.Root bind:open onOpenChange={handleOpenChange}>
      <Popover.Trigger
        class="flex w-full items-center justify-between rounded-sm border border-border bg-background px-3 py-2 text-sm text-foreground shadow-sm transition-colors hover:bg-accent/50 focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
      ...
      >
        ...
        {:else}
          <span class="truncate text-muted-foreground"
            >{m.wishlist_picker_search_placeholder()}</span
          >
        {/if}
