...
-->
<Card
  class="group preview-card relative w-full rounded-2xl border-2 border-primary/35 shadow-xl transition-all duration-200 hover:border-primary hover:shadow-[0_0_10px_rgba(212,138,66,0.15)] {isSold
...
>
  ...
  <CardContent class="p-3 sm:pt-0">
    <!--
      Centerpiece: "technical drawing" area.
      Subtle grid pattern via inline background-image; bg-zinc-900 as base.
    -->
    <div class="flex flex-row items-start gap-4 sm:block">
      <div
        class="relative aspect-video h-32 w-32 shrink-0 overflow-hidden rounded-xl bg-card saturate-[0.9] transition-all duration-200 group-hover:saturate-100 sm:h-auto sm:w-full"
      ...
      >
        ...
        {:else}
          ...
          <div class="flex h-full w-full items-center justify-center">
            <PlaceholderIcon class="size-10 text-zinc-600/70 sm:size-12" />
          </div>
        {/if}
      ...
      <div class="min-w-0 flex-1">
        <Separator class="my-2.5 hidden bg-border sm:block" />

        <div class="space-y-2 sm:space-y-3">
          <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs sm:grid-cols-[2fr_1fr_1fr] sm:gap-3">
            <div class="flex min-w-0 flex-col items-start gap-0.5 text-left sm:items-center sm:text-center">
          ...
          {#if displayPurchaseDate || displayPrice}
            <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs sm:grid-cols-2 sm:gap-3">
              <div class="flex min-w-0 flex-col items-start gap-0.5 text-left sm:items-center sm:text-center">
                <span class="text-[10px] tracking-wider text-muted-foreground uppercase">
