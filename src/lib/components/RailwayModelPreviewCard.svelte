...
<Card
	class="card gauge-frame ring-1 ring-border/40 transition-all duration-200 hover:scale-[1.02] hover:ring-2 hover:ring-primary-500 {className ||
		''}"
>
	{#if onDelete}
	...
	<CardContent class="p-4 {onDelete ? 'pt-2' : ''}"
		...
		<div class="grid gap-4 sm:grid-cols-1 lg:grid-cols-[auto_1fr]">
			...
			<div class="relative aspect-video w-full overflow-hidden rounded-lg bg-surface-200 lg:w-48">
				{#if model.photoUrl}
					<img
						src={model.photoUrl}
				...
				{#if model.unitCount && model.unitCount > 1}
					<div
						class="absolute bottom-2 right-2 z-10 rounded-full bg-black/60 px-2 py-1 text-xs font-medium text-white"
				...
				<div class="mt-1">
					{#if shouldTruncateRoadNumber}
						<button
							...
							class="rounded bg-surface-100 px-2 py-1 font-mono text-sm hover:bg-surface-200"
						>
							# {roadNumberExpanded ? model.roadNumber : displayRoadNumber}
						</button>
					{:else}
						<span class="inline-block rounded bg-surface-100 px-2 py-1 font-mono text-sm">
							# {displayRoadNumber}
						</span>
					{/if}
				</div>
